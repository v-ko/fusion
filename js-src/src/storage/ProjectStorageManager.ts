...
export interface ProjectStorageConfig {
    ...
    inMemoryRepoIndexConfigs: readonly IndexConfig[];
...
async function initStorageAdapter(config: StorageAdapterConfig): Promise<BaseAsyncRepository> {
    ...
    switch (config.name) {
        ...
        case "InMemory": { // For testing purposes
            let inMemRepo = new AsyncInMemoryRepository(config.args.indexConfig);
...
export class ProjectStorageManager {
    ...
    async init() {
        ...
        try {
            ...
            this._inMemRepo = await AsyncInMemoryRepository.initFromRemote(this.localStorageRepo, branchName, this.config.inMemoryRepoIndexConfigs)
